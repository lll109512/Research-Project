...
from Modals import OneLayerCNN, BiLSTMmax, BiMultiLSTMmax, OneLayerCNNWithBiLSTMMax, DCNN, BiLSTMmaxWithMultichannel,BiMultiLSTMmaxWithMultichannel
import config
...
class Train:
    ...
    def train(self, s1, s2, labels, train_datasize, ts1, ts2, tlabels, ttrain_datasize):
        ...
        Arguments:
            s1 {dict} -- training dataset sentence 1
            s2 {dict} -- training dataset sentence 2
            labels {dict} -- training dataset labels
            train_datasize {int} -- training dataset size
            ts1 {dict} -- test dataset sentence 1
            ts2 {dict} -- test dataset sentence 2
            tlabels {dict} -- test dataset labels
            ttrain_datasize {int} -- test dataset size
        """

        for i in range(iterations):
            s1arr, s2arr,labelarr = ld.get_batch(s1, s2, labels, train_datasize,corrupt=self.useCorrupt)
            self.sess.run([self.optimizer], {self.s1_input: s1arr, self.s2_input: s2arr, self.labels: labelarr,self.step:i})
            if (i % 100 == 0):
    ...
    def embedding_sentence(self, sentences):
        """calculate embedding of given sentences
        
        Arguments:
            sentences {list} -- a list of sentences
        
        Returns:
            numpy array -- embedding sentences list
        """
    ...
    def test(self,s1,s2,labels,datasize):
        ...
        Arguments:
            s1 {dict} -- test sentence 1
            s2 {dict} -- test sentence 2
            labels {dict} -- lables 
            datasize {int} -- size of test data size
        """
